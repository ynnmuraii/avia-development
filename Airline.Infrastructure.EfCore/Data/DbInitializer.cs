...
/// <summary>
/// Метод расширения для инициализации базы данных с миграциями и заполнением данных.
/// </summary>
public static class DbInitializer
{
    ...
    public static async Task InitializeAsync(this AirlineDbContext context, ILogger logger = null)
    {
        ...
        if (!await context.AircraftFamilies.AnyAsync())
        {
            ...
            var models = GetAircraftModels(families);
            ...
            var flights = GetFlights(models);
            await context.Flights.AddRangeAsync(flights);
            await context.SaveChangesAsync();

            var tickets = GetTickets(flights, passengers);
